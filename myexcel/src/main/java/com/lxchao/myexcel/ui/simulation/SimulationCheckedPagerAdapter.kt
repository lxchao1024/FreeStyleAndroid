 ...
 * @date 2019/11/18
 */
class SimulationCheckedPagerAdapter
/**
 * 构造函数
 * 初始化上下文和数据
 *
...
(//上下文
        ...
        private val mData: List<DuoXuan>) : PagerAdapter() {
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        ...
        title.text = "题目名称： " + data.题目名称
        var mContent = "问题：\n" + data.题目内容 + "\nA: " + data.选项Ａ + "\nB: " + data.选项B + "\nC: " + data.选项C + "\nD: " + data.选项D
        if (data.选项E.length > 0) {
            mContent += "\nE: " + data.选项E
        }
        tv.text = mContent

        answer.text = "答案：" + data.正确答案
        val pos = view.findViewById<TextView>(R.id.tv_item_pos)
        pos.text = "当前进度：${position + 1} / ${mData.size}"
        SpUtils.put(SpUtils.KEY_SIMULATION_CHECKED, position)
